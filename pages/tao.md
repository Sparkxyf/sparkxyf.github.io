...
<ul class="listing">
{% for tao in site.tao %}
{% if tao.title != "tao Template" and tao.topmost == true %}
<li class="listing-item"><a href="{{ site.url }}{{ tao.url }}"><span class="top-most-flag">[置顶]</span>{{ tao.title }}</a></li>
{% endif %}
{% endfor %}
{% for tao in site.tao %}
{% if tao.title != "tao Template" and tao.topmost != true %}
<li class="listing-item"><a href="{{ site.url }}{{ tao.url }}">{{ tao.title }}<span style="font-size:12px;color:red;font-style:italic;">{%if tao.layout == 'mindmap' %}  mindmap{% endif %}</span></a></li>
{% endif %}
{% endfor %}
</ul>
